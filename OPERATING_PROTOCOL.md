...
Consult the [twelve upstream research agents](meta/UPSTREAM_RESEARCH_AGENTS.md) and [agent registry](meta/UPSTREAM_AGENT_REGISTRY.json) alongside the existing GEV/weather/energy intake. At every existing cycle, first inspect complete staged packets under `evidence/research/pending/` using the [pending-packet intake contract](meta/RESEARCH_REPORT_INTAKE.md#cloud-pending-packets), then review received complete reports by the evidence cutoff, record relevant, unchanged, stale, missing and inconclusive inputs, and audit common source ancestry before any model interpretation. Verify packet bytes and record actual complete market receipt time; a staging commit alone is not receipt, acknowledgment or canonical intake. A packet first read after the cycle cutoff is eligible only for a later cutoff; never backdate it. Setup configuration itself is not a research report and triggers no extra cycle.
