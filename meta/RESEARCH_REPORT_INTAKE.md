...
The three existing producers retain their method repositories (`satellite-research`, `weather`, and lowercase `energy`) and Monday 08:00, 08:20 and 08:40 Pacific starts. Their cloud runtime, checkpoints and immutable report packages are prepared in the existing private `6t8hggfm44-tech/-Energy` repository under `cloud/legacy-weekly/`. Preparation is not verified activation; inspect each producer's actual deployment state. No report is created or accepted by this setup change.
